...
# this class name is a Little Elephpant
// write by PHP v7.4 & SQL v8

class LE extends PDO {
...
// =========================
    ...
    private function binding () {
        if (!$this->parameters || !$this->values) { return; }
    ...
    private function run_sql (int $outputType=self::OP_NO_CHANGE, bool $echoQuery=false) {
        ...
        switch ($outputType) {
            ...
            default:
                ...
                // actual run
                $this->finish ($sql, $outputType); break;
        }
    }
...
// SECTION ADD RANDOM DATA
    ...
    private function ard_split (array $mainData, array $components) : array {
        foreach ($components as $k => $v) {
            switch ($v[0]) {
                case 's':
                    $mainData[$k] = $this->rand_str ($v[1], $v[2]); break;

                case 'e':
                    $mainData[$k] = $this->rand_str ($v[1], $v[2], '@email.ir'); break;

                case 'i':
                    $mainData[$k] = $this->rand_int($v[1]); break;

                case 't':
                    $mainData[$k] = date('Y/m/d H:i:s'); break;

                case 'f':
                    $mainData[$k] = $v[1]; break;

                case 'r':
                    $mainData[$k] = $this->random_number ($v[1]); break;
            }
        }
...
// =========================
    ...
    private function op_no_change () {
        ...
        if ($R = $this->result->fetchAll ()) { return $R; }
